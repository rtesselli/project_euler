...
def problem37():
    ...
    starts = np.frompyfunc(lambda x, y: str.startswith(x, y) and len(x) == len(y) + 1, 2, 1)
    ends = np.frompyfunc(lambda x, y: str.endswith(x, y) and len(x) == len(y) + 1, 2, 1)
    start_matrix = starts(primes.reshape(1, -1), primes.reshape(-1, 1))
    end_matrix = ends(primes.reshape(1, -1), primes.reshape(-1, 1))
    start_matrix = start_matrix & ~np.eye(len(primes), dtype=bool)
    end_matrix = end_matrix & ~np.eye(len(primes), dtype=bool)
